...
contract MasterChefHelper {
    ...
    // swapTokenForPoolToken(1, dai, daiIn, 0); daiIn = 50 * 10 **18
    function swapTokenForPoolToken(uint256 poolId, address tokenIn, uint256 amountIn, uint256 minAmountOut) external {
        ...
        address tokenOut0 = UniswapV2PairLike(lpToken).token0();//USDC
        ...
        _swap(tokenIn, tokenOut0, amountIn / 2);//用25个dai换出usdc
        _swap(tokenIn, tokenOut1, amountIn / 2);//用25个dai换出weth

        // add liquidity and give lp tokens to msg.sender
        _addLiquidity(tokenOut0, tokenOut1, minAmountOut);//将usdc和weth全部加入流动池中
    }
